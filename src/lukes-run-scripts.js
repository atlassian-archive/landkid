...
const axiosGetConfig = { auth: { username: BB_USERNAME, password: BB_PASSWORD } };
const axiosPostConfig = { ...axiosGetConfig, headers: { 'Content-Type': 'application/json'} }
...
function processCommentWebhook(data) {
  ...
  const commentBody = data && data.comment && data.comment.content && data.comment.content.raw;
...
async function startCustomPipeline(pullRequestId, customBuildName) {
  const endpoint = `https://api.bitbucket.org/2.0/repositories/${REPO_OWNER}/${REPO_SLUG}/pipelines/`;
  ...
  const response = await axios.post(endpoint, JSON.stringify(postData), axiosPostConfig);
  return response.data;
}

async function getPullRequestToCommit(pullRequestId) {
  const endpoint = `https://api.bitbucket.org/2.0/repositories/${REPO_OWNER}/${REPO_SLUG}/pullrequests/${pullRequestId}`;
  const response = await axios.get(endpoint, axiosGetConfig);
  const commit = response.data && response.data.source && response.data.source.commit && response.data.source.commit.hash;
  return commit;
}
...
function createWebhook(ngrokUrl) {
  ...
  const endpoint = `https://api.bitbucket.org/2.0/repositories/${REPO_OWNER}/${REPO_SLUG}/hooks`;
  const webhookData = {
  ...
  return new Promise(resolve => axios.post(endpoint, webhookData, axiosGetConfig)
      .then((res) => {
...
async function cleanUpOldWebhooks() {
  ...
  return axios.get(endpoint, {...axiosGetConfig, params: { pagelen: 30 }})
    .then((response) => {
      ...
      return Promise.all(currentUserWebhooks.map(webhook => deleteWebhook(webhook)))
        .then((results) => {
...
function deleteWebhook(webhookEndpoint) {
  ...
  return axios.delete(webhookEndpoint, axiosGetConfig)
